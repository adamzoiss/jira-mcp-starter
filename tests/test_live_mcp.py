"""Optional end-to-end MCP transport tests that talk to the server over stdio."""
...
# Load local settings so the test can spawn the MCP server with real Jira credentials.
...
class JiraMcpLiveIntegrationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        # Skip unless the caller has provided a real issue key for live validation.
    ...
    def test_mcp_server_lists_expected_tools(self) -> None:
        # Tool discovery validates that the server boots and completes MCP initialization.
    ...
    def test_mcp_get_issue_returns_expected_shape(self) -> None:
        # This hits the actual MCP tool call path rather than the Jira client directly.
    ...
    def test_mcp_search_issues_returns_expected_shape(self) -> None:
        # Search shape must remain stable over the real transport for Codex consumers.
    ...
    async def _list_tool_names(cls) -> list[str]:
        # Each helper creates a fresh MCP session so tests remain independent.
    ...
    async def _call_tool(cls, tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        # MCP tool calls may return structured content or a transport-level tool error.
        if result.isError:
    ...
    def _server_parameters(cls) -> StdioServerParameters:
        # Pass only the variables the server actually needs instead of cloning the whole env.
        env = {
        ...
        return StdioServerParameters(
            # Spawn the same server module a real MCP client would invoke locally.
...
class _ClientSessionContext:
    """Tiny async context manager that owns the stdio transport and MCP session lifecycle."""

    def __init__(self, server_params: StdioServerParameters) -> None:
        self.server_params = server_params

    async def __aenter__(self) -> ClientSession:
        # The transport is created first, then wrapped by the higher-level MCP client session.
        self._stdio_context = stdio_client(self.server_params)
        read_stream, write_stream = await self._stdio_context.__aenter__()
        self._session = ClientSession(read_stream, write_stream)
        # Initialization performs the MCP handshake and discovers server capabilities.
    ...
    async def __aexit__(
        ...
        # Close the logical session before tearing down the underlying stdio process.
